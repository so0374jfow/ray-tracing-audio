...
function reflectRayFromCircle(ray, circle, intersectionPoint) {
  const reflectedVector = reflectionVector(
    ray.vector,
    normalize(circle.center.subtract(intersectionPoint))
  );
  return new Ray(intersectionPoint, reflectedVector);
}
...
function rayMapper(ray) {
  let rayMeta = [...Scene.finishedLines, ...Scene.circles].reduce(rayIntersectReducer(ray), {
...
function checkSceneIntersections() {
  ...
  Scene.primaryRays.map(rayMapper).forEach((result, index) => {
